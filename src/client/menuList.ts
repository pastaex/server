  ...
  showAdminMenu: function () {
    ...
    menu.ItemSelect.on(async (item) => {
      ...
      if (item.doName == 'enableAdmin') {
         user.setVariable('enableAdmin', true), user.godmode = true;
         mp.game.ui.notifications.show('AdminMode: ~g~On');
         UIMenu.Menu.HideMenu();
      };
      if (item.doName == 'disableAdmin') {
        ...
        mp.game.ui.notifications.show('AdminMode: ~r~Off');
        UIMenu.Menu.HideMenu();
      };
...
mp.keys.bind(0x37, true, function() {
  ...
  mp.events.call('client:teleportWaypoint'); // ТП НАХУЙ Я ТВОЙ РОТ ШАТАЛ 
