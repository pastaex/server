...
import LiteEvent, { CheckboxChangeEvent, ItemSelectEvent, ListChangeEvent } from "./utils/LiteEvent";
...
export default class NativeUI {
  ...
  workAnyTime: boolean;
	...
	constructor(title:string, subtitle:string, offset:{X:number,Y:number}, spriteLibrary:string, spriteName:string) {
		...
		mp.console.logInfo(`Created Native UI! ${this.title}`);
	}
	...
	public Close() {
		...
		this.Visible = false;
		this.RefreshIndex();
		if(this.serverID) this.MenuClose.emit(this.serverID);
	...
	public SelectItem() {
		if((methods.isBlockKeys() && !this.workAnyTime) || Menu.Menu.IsShowInput()) return;
	...
	public ProcessControl() {
		...
		if (mp.game.controls.isControlJustReleased(0, 177) && !Menu.Menu.IsShowInput()) {
			// Back
			this.GoBack();
		}
...
export {NativeUI};
export {UIMenuItem};
export {UIMenuListItem};
export {UIMenuCheckboxItem};
export {UIMenuSliderItem};
export {BadgeStyle};
export {Point};
export {Size};
export {Color};
