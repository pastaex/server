...
interface Mp extends CefMp {
  console: {
    clear():void;
    reset():void;
    logInfo(message: string, 
      /** Saves message into a text file in the RAGEMP folder */
      save?: boolean, 
      /** Saves message into a text file in the RAGEMP folder asynchronous */
      saveAsync?:boolean
    ):void;
...
interface EntityMp {
  playSound: (url: string, volume: number, dist: number) => void;
  alpha: number;
  dimension: number;
  model: number;
  position: Vector3Mp;
...
interface PlayerMp extends PlayerClient {
  deliverVehWait: boolean;
    isSpeaking: boolean;
    isListening: boolean;
  __attachmentObjects: {
    [id:number]:ObjectMp;
  ...
  taskEnterVehicle(
    ...
    speed: number,
    /** Режим посадки в ТС
     * + 1 Moves to the vehicle
     * + 3 Teleport to the vehicle
     * + 16 Teleports directly intro vehicle
     */
    mode: 1|3|16,
    p6: any
  ): void;
...
interface VehicleMp extends EntityMp {
  ...
  isSirenOn(): boolean;
  setSirenSound(status:boolean): void;
...
interface BrowserMp extends ClientBrowserMp {
  active: boolean;
  url: string;
...
interface VoiceChatMp {
  cleanupAndReload(arg0: boolean, arg1: boolean, arg2: boolean): any;
  muted: boolean;
...
interface GameFireMp {
  addExplosion(x: number, y: number, z: number, arg3: number, arg4: number, arg5: boolean, arg6: boolean, arg7: number):any;
  addSpecfxExplosion(
    x: number,
    y: number,
    z: number,
...
interface GamePlayerMp {
  setWeaponDefenseModifier(param: number): void;
...
interface GameUiMp {
  ...
  notifications: {
    show(message: string, time?:number): any;
    showWithPicture(title: string, sender: string, message: string, notifPic: string, icon?: number, flashing?: boolean, textColor?: number, bgColor?: number, flashColor?: [number, number, number, number]): any
  }
}
...
interface EventMpPool extends CustomEventPool {
  callSocket: (event: string, ...args: any[]) => void;
  containerLocal: Map<number, RegisterLocalCallback>;
  eventID: number;
  eventRemoteRequestData: Map<number, any>;
  eventRemoteRequestId: number;
  container: Map<number, { id: number; name: string; func: RegisterCallback; }>;
