...
methods.createExplopeCheckpoint(new mp.Vector3(999.67, 29.53, 71.76), -1042390945, player => {
  if (!testMode && weather.getHour() > 6) {
    player.notify("~r~Доступно только с 0 до 6 утра игрового времени");
    return false;
  }
  if (methods.explodedDoors.find((item) => item[3] == -1042390945)){
...
methods.createEnterCheckpoint(new mp.Vector3(997.90, 26.56, 71.74), player => {
  if (methods.explodedDoors.find((item) => item[3] == -1042390945)) return;
...
pacificPoss.map((q, index) => {
  ...
  methods.createDynamicCheckpoint(pacificPos, "Нажмите ~g~E~w~ чтобы ограбить ячейку №" + (index+1), player => {
    ...
    if (!methods.explodedDoors.find((item) => item[3] == -1042390945)) return player.notify("~r~Дверь не взорвана")
