...
let isTimeForTakeGift = (data.getMonth() == 10 && data.getDate() <= maxDayTake)
 ...
 setInterval(() => {
   data = new Date();
  isTimeForGiveGift = (data.getMonth() == 10 && data.getDate() >= minDayGive)
   isTimeForTakeGift = (data.getMonth() == 10 && data.getDate() <= maxDayTake)
 }, 60000)
...
async function mainGift(player:PlayerMp){
  let giftsend = await mysql.executeQuerys("SELECT * FROM `christmas_gift` WHERE `user` = ?", [user.getId(player)])
...
async function getGift(player:PlayerMp){
  let giftsend = await mysql.executeQuerys("SELECT * FROM `christmas_gift` WHERE `target` = ?", [user.getId(player)]);
    ...
    m.newItem({
      ...
      onpress: () => {
        ...
        m.close();
          ...
          mysql.executeQuerys("UPDATE `christmas_gift` SET `received` = ? WHERE `id` = ?", [1, data.id]).then(() => {
            ...
            menu.input(player, "Подпишите подарок", "Anonim", 100).then(name => {
              ...
              menu.input(player, "Добавьте записку к подарку", "", 500).then(note => {
                ...
                menu.input(player, "Введите сумму", "1000", 1000, "int").then(moneys => {
                  ...
                  mysql.executeQuerys("INSERT INTO `christmas_gift` (`user`, `target`, `name`, `item`, `note`) VALUES (?, ?, ?, ?, ?)", [user.getId(player), id, name, JSON.stringify({id:0,amount:money}), note]).then(() => {
              ...
              user.checkIdUser(id).then(check => {
                ...
                menu.input(player, "Подпишите подарок", "Anonim", 100).then(name => {
                  ...
                  menu.input(player, "Добавьте записку к подарку", "", 1000).then(note => {
                    ...
                    mysql.executeQuerys("INSERT INTO `christmas_gift` (`user`, `target`, `name`, `item`, `note`) VALUES (?, ?, ?, ?, ?)", [user.getId(player), id, name, JSON.stringify({id:item.item_id,amount:1}), note]).then(() => {
